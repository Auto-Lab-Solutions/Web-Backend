...
def authorize(event, role):
    ...
    try:
        ...
        # Check if staff has required roles
        if not staff_roles or role not in staff_roles:
            print(f'Staff roles: {staff_roles}, Required role: {role}')
            return auth.generate_policy(
                principal_id='unauthorized',
                effect='Deny',
                resource=resource,
                context={'errorMessage': 'Staff does not have required roles'}
            )
