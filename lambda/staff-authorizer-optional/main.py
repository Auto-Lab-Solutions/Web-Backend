...
def lambda_handler(event, context):
    """
    Optional staff authorizer with enhanced error handling and logging
    Allows access even when no token is provided
    """
    try:
        ...
        if not token:
            print("No token provided - allowing unauthenticated access")
        ...
        print(f'Critical error in optional staff authorizer: {str(e)}')
        # In case of critical error, allow access since this is optional auth
        return auth.generate_policy(
            principal_id='error_fallback',
            effect='Allow',
            resource=event.get('methodArn', '*'),
            context={'infoMessage': 'Authorization error - fallback to allow'}
        )
