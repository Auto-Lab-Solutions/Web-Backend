...
show_usage() {
    ...
    echo "  REPO_OWNER                  Owner of the GitHub repository."
    echo "  REPO_NAME                   Name of the GitHub repository."
    ...
    echo "\nExample:"
    echo "  FRONTEND_GITHUB_TOKEN=ghp_... REPO_OWNER=Auto-Lab-Solutions REPO_NAME=Web-Frontend ENVIRONMENT=production STACK_NAME=your-stack $0"
...
echo "FRONTEND_GITHUB_TOKEN: $FRONTEND_GITHUB"
...
if [ -z "$FRONTEND_GITHUB_TOKEN" ] || [ -z "$REPO_OWNER" ] || [ -z "$REPO_NAME" ] || [ -z "$ENVIRONMENT" ] || [ -z "$STACK_NAME" ] || [ -z "$STRIPE_PUBLISHABLE_KEY" ]; then
  echo "Error: FRONTEND_GITHUB_TOKEN, REPO_OWNER, REPO_NAME, ENVIRONMENT, STACK_NAME, or STRIPE_PUBLISHABLE_KEY is not set."
  show_usage
  exit 1
fi
...
for VAR_NAME in "${!ENV_VARS[@]}"; do
  ...
  echo "Setting environment variable $VAR_NAME in $REPO_OWNER/$REPO_NAME/$ENVIRONMENT..."
  ...
  VARS_JSON=$(curl -s -H "Authorization: Bearer $FRONTEND_GITHUB_TOKEN" \
    "https://api.github.com/repos/$REPO_OWNER/$REPO_NAME/environments/$ENVIRONMENT/variables")
  ...
  if [ -z "$VAR_ID" ] || [ "$VAR_ID" == "null" ]; then
    ...
    if [ "$RESP" -ge 200 ] && [ "$RESP" -lt 300 ]; then
      echo "Created environment variable $VAR_NAME in $REPO_NAME/$ENVIRONMENT."
    else
    ...
    RESP=$(curl -s -w "%{http_code}" -o /tmp/gh_var_resp.json -X PATCH \
      ...
      -H "Content-Type: application/json" \
      -d "{\"name\":\"$VAR_NAME\",\"value\":\"$VAR_VALUE\"}" \
      "https://api.github.com/repos/$REPO_OWNER/$REPO_NAME/environments/$ENVIRONMENT/variables/$VAR_ID")
    if [ "$RESP" -ge 200 ] && [ "$RESP" -lt 300 ]; then
      echo "Updated environment variable $VAR_NAME in $REPO_NAME/$ENVIRONMENT."
    else
...
echo "All environment variables have been set successfully in $REPO_NAME/$ENVIRONMENT."
