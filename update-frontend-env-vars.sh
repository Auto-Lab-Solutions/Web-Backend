...
if [ -z "$FRONTEND_GITHUB_TOKEN" ] || [ -z "$FRONTEND_REPO_OWNER" ] || [ -z "$FRONTEND_REPO_NAME" ] || [ -z "$ENVIRONMENT" ] || [ -z "$STACK_NAME" ] || [ -z "$STRIPE_PUBLISHABLE_KEY" ] || [ -z "$MAIL_FROM_ADDRESS" ]; then
  echo "Error: FRONTEND_GITHUB_TOKEN, FRONTEND_REPO_OWNER, FRONTEND_REPO_NAME, ENVIRONMENT, STACK_NAME, STRIPE_PUBLISHABLE_KEY, or MAIL_FROM_ADDRESS is not set."
  show_usage
  exit 1
fi
